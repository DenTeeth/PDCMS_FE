...
public interface DentalServiceRepository extends JpaRepository<DentalService, Long> {
        ...
        * FIXED: Explicit CAST to avoid PostgreSQL "function lower(bytea) does not
        * exist" error
        */
       @Query("SELECT s FROM DentalService s " +
                     "LEFT JOIN FETCH s.category c " +
                     ...
                     "     LOWER(CAST(s.serviceName AS string)) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR " +
                     "     LOWER(CAST(s.serviceCode AS string)) LIKE LOWER(CONCAT('%', :searchTerm, '%')))")
