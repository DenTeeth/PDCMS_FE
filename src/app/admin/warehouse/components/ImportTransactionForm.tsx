  ...
  const mutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      toast.success('Nhập kho thành công!', {
        description: `Mã phiếu: ${response.transactionCode} | Tổng giá trị: ${response.totalValue.toLocaleString('vi-VN')} VNĐ | Trạng thái: ${response.status === 'PENDING_APPROVAL' ? 'Chờ duyệt' : response.status}`,
        duration: 5000,
      });
