...
export default function AdminTimeOffRequestsPage() {
  ...
  // Permissions - Using new BE consolidated permissions
  // BE uses VIEW_TIME_OFF_ALL/OWN (not VIEW_LEAVE_ALL/OWN)
  const canViewAll = user?.permissions?.includes('VIEW_TIME_OFF_ALL');
  const canViewOwn = user?.permissions?.includes('VIEW_TIME_OFF_OWN');
  const canCreate = user?.permissions?.includes('CREATE_TIME_OFF');
  const canApprove = user?.permissions?.includes('APPROVE_TIME_OFF');
  const canReject = user?.permissions?.includes('APPROVE_TIME_OFF'); // BE uses same permission for approve/reject
  const canCancelPending = user?.permissions?.includes('APPROVE_TIME_OFF'); // Admin can cancel pending
  const canCancelOwn = user?.permissions?.includes('CREATE_TIME_OFF'); // User can cancel own
  ...
  return (
    <ProtectedRoute requiredBaseRole="admin" requiredPermissions={['VIEW_TIME_OFF_ALL']}>
