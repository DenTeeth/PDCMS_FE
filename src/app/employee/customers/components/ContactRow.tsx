    ...
    <tr>
      ...
      <td className="py-2 px-3">
        <div className="flex items-center gap-2">
          <Link href={`${basePath || '/employee/customers/contact'}/${contact.id}`} className="text-sm px-2 py-1 rounded bg-blue-50 text-blue-600">Xem</Link>
          <Link href={`${basePath || '/employee/customers/contact'}/${contact.id}/edit`} className="text-sm px-2 py-1 rounded bg-green-50 text-green-600">Sửa</Link>
          {visibleDelete && (
            <button onClick={handleDelete} className="text-sm px-2 py-1 rounded bg-red-50 text-red-600">Xóa</button>
          )}
        </div>
      </td>
    </tr>
